...
const DirectChatDialog: React.FC<DirectChatDialogProps> = ({
  open,
  onOpenChange,
  currentUserId,
  partnerId,
  ...
  const { messages, isLoading, sendMessage } = useChatMessages(currentUserId, open ? partnerId : null);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md h-[70vh] flex flex-col p-0 rounded-2xl">
        <DialogHeader className="p-4 border-b bg-gradient-hero rounded-t-2xl">
          <DialogTitle className="flex items-center gap-2 text-primary-foreground text-sm">
        ...
        <ScrollArea className="flex-1 p-4" ref={scrollRef}>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-muted-foreground text-xs">Say hello to {partnerName}!</p>
            </div>
          ) : (
                    ...
                    <div className={cn("max-w-[75%] flex flex-col", isOwn ? "items-end" : "items-start")}>
                      <div
                        className={cn(
                          ...
                          isOwn ? "bg-gradient-primary text-primary-foreground rounded-br-md" : "bg-muted rounded-bl-md"
                        )}
                      >
                        {msg.message}
                      </div>
