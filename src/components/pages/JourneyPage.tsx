...
const JourneyPage: React.FC<JourneyPageProps> = ({ onNavigateToAccount, externalActivities = [] }) => {
  ...
  const handleSaveActivity = (activityData: Omit<Activity, "id"> & { id?: number }) => {
    if (activityData.id) {
      setActivities((prev) => prev.map((a) => (a.id === activityData.id ? { ...a, ...activityData } : a)));
    } else {
      setActivities((prev) => [...prev, { ...activityData, id: Date.now() }]);
    }
  };

  // Mark complete + undo option
  const handleCompleteActivity = (id: number) => {
    setActivities((prev) => prev.map((a) => (a.id === id ? { ...a, status: "completed" } : a)));
    setRecentlyCompleted(id);
    toast.success("Activity marked as complete! Click Undo to revert.", {
      action: {
        label: "Undo",
  ...
  const handleUndoComplete = (id: number) => {
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex-1 overflow-y-auto">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col h-full">
          <TabsList className="grid w-[calc(100%-2rem)] grid-cols-2 mx-4 mt-3 h-11 rounded-xl bg-muted">
            <TabsTrigger value="planner" className="text-xs rounded-lg flex items-center gap-1.5">
              <Calendar className="w-4 h-4" />
              Planner
            </TabsTrigger>
            <TabsTrigger value="expenses" className="text-xs rounded-lg flex items-center gap-1.5">
          ...
          <TabsContent value="planner" className="flex-1 overflow-y-auto px-4 pt-3 pb-20">
            ...
            <Card className="p-3 mb-4 bg-gradient-card rounded-2xl border-0 shadow-soft">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <h3 className="font-semibold text-sm">Current Trip: {trip.destination}</h3>
                  <p className="text-[10px] text-muted-foreground">
                ...
                >
                  <div className="flex gap-3">
                    <div
                      className={cn(
                        "w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0",
                        activity.status === "completed" ? "bg-success/20" : "bg-primary/10",
                      )}
                    >
