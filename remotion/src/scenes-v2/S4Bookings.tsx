...
export const S4Bookings = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  const phoneS = spring({ frame, fps, config: { damping: 14 } });
  const phoneScale = interpolate(phoneS, [0, 1], [0.3, 1]);
  ...
  const features = [
    { icon: "🏨", text: "AI hotel search" },
    { icon: "⭐", text: "Filter by rating" },
    { icon: "💵", text: "Compare prices" },
    { icon: "📍", text: "Location-based results" },
  ];
  ...
  return (
    <AbsoluteFill style={{
      ...
      <div style={{
        ...
        <IPhoneFrame screenSrc="images/screens/bookings.png" scale={1.1} />
      </div>
    </AbsoluteFill>
  );
};
